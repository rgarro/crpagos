...
jQuery(function($) {
	...
	function processModal(settings) {
		...
		if (from) {
			if (currentSettings.type == 'form') {
				...
				initModal();
				if (from.href == undefined){from.href = from.action};
	...
	function wrapContent() {
		...
		if (currentSettings.type == 'gallery') {
			...
			var gallery = $('[rel="'+currentSettings.from.rel+'"]');
			var currentIndex = gallery.index(currentSettings.from);
//fix to select the current radio
			if (document.TheForm.tarjetaid) {
				document.TheForm.tarjetaid[currentIndex].checked = true;
			}
			if (currentIndex > 0) {
	...
	$($.fn.nyroModal.settings.openSelector).nyroModal();	
		...
		preloadImg('/js/nyroModal/images/ajaxLoader.gif');
		preloadImg('/js/nyroModal/images/prev.gif');
		preloadImg('/js/nyroModal/images/next.gif');
