...
$session = $this->request->session();
	if(count($session->read('Companies')) > 1){
		echo '<small>',__('MultiCompanyUser'),'</small><br><select style="font-size:10px;;width:165px" name="Avail" OnChange="if(this.options[this.selectedIndex].value!=\'\'){companyChange(this.options[this.selectedIndex].value);}">',"\n";
