...
//Button Row
				...
				if(isset($ResendOK)){
					echo '<input type="Submit" name="Resend" value="',($ThisInvoice['Status']['StatusID']==2)?  __('SendMail'): __('ResendMail'),'">';
					unset($ResendOK);
				}else{
					echo '&nbsp;';
				}
...
//close previous div and tables
			...
			echo "\n";
			echo '<div id="tabs-',$ThisInvoice['Invoices']['StatusID'] ,'">';
			if($ThisInvoice['Invoices']['StatusID'] < 3){
				...
				$ResendOK = "Ok";
			}elseif($ThisInvoice['Invoices']['StatusID'] == 5){
		...
		if($ThisInvoice['Invoices']['StatusID'] < 3){
			echo '<input type="checkbox" name="InvoiceID[]" class="radios" value="',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'">';
		}elseif($ThisInvoice['Invoices']['StatusID'] == 5){
			echo '<input type="checkbox" name="InvoiceID[]" class="radios" value="',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'">';
		}else{
			echo '&nbsp;';
		...
		if($session->read('LocaleCode') == 'spa_cr'){
			echo ucwords(strftime('%A %d de %B del %Y',strtotime($ThisInvoice['Invoices']['InvoiceDate'])));
		}else{
			echo ucwords(strftime('%A, %B %d %Y',strtotime($ThisInvoice['Invoices']['InvoiceDate'])));
		}
		echo '</td>';
		echo '<td align="center">',$ThisInvoice['Invoices']['InvoiceNumber'],'</td>';
		echo '<td style="font-size:0.8em">',$ThisInvoice['Clients']['ClientName'],' ',$ThisInvoice['Clients']['ClientLastName'],' (',$ThisInvoice['Clients']['Email'],')</td>';
		...
		if($ThisInvoice['Invoices']['StatusID'] == 1){
//Pending: Edit all
			echo '&bull;<a href="',$session->read('Company.CurrentCompanyURL'),'editinvoice/',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'/">',__('EditInvoice'),'</a>';
		}elseif($ThisInvoice['Invoices']['StatusID'] == 2){
//Sent: PayManually / Void
			echo '&bull;<a href="',$session->read('Company.CurrentCompanyURL'),'payinvoice/',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'/">',__('PayLink'),'</a>';
			echo '<br>';
			echo '&bull;<a href="',$session->read('Company.CurrentCompanyURL'),'voidinvoice/',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'/">',__('VoidLink'),'</a>';
		}else{
//Paid: Read Only
			echo '&bull;<a href="',$session->read('Company.CurrentCompanyURL'),'viewinvoice/',base64_encode($ThisInvoice['Invoices']['InvoiceID']),'/">',__('ViewLink'),'</a>';
		}
		echo '</td>';
		echo '</tr>';
		$CurrentRow++;
